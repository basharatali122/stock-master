...
export const productSchema = z.object({
  ...
  price: z.number().positive({ message: 'TP Rate must be greater than 0' }).max(10000000, { message: 'Price exceeds maximum limit' }),
  purchase_rate: z.number().min(0, { message: 'Purchase rate cannot be negative' }).max(10000000, { message: 'Purchase rate exceeds maximum limit' }),
